-- Bei Paarprofil: Daten der zweiten Person
alter table public.profiles
  add column if not exists partner_nick text,
  add column if not exists partner_date_of_birth date;

comment on column public.profiles.partner_nick is 'Nur bei Paar-Account: Nick/Anzeigename der zweiten Person';
comment on column public.profiles.partner_date_of_birth is 'Nur bei Paar-Account: Geburtsdatum der zweiten Person';

-- Trigger: partner_nick und partner_date_of_birth aus user_metadata
...
begin
  if v_account_type = 'couple' then
    ...
    v_gender := 'Divers';
    v_role := 'Switcher';
    v_partner_nick := nullif(trim(new.raw_user_meta_data->>'partner_nick'), '');
  ...
  insert into public.profiles (id, nick, gender, role, date_of_birth, account_type, couple_type, partner_nick, partner_date_of_birth)
  values (
    new.id,
