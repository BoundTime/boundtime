-- Supabase legt pgcrypto im Schema "extensions" an. Die RPCs laufen mit search_path = public
-- und finden gen_salt/crypt dort nicht → "function gen_salt(unknown) does not exist".
-- Lösung: extensions.gen_salt und extensions.crypt explizit verwenden.
...
begin
  ...
  return v_hash = crypt(v_pwd, v_hash);
end;
$$;
...
begin
  ...
  update public.profiles set
    restriction_password_hash = case when v_pwd_trim is not null then crypt(v_pwd_trim, gen_salt('bf')) else restriction_password_hash end,
