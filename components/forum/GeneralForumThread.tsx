...
import { GeneralForumPostDeleteButton } from "@/components/forum/GeneralForumPostDeleteButton";
...
export function GeneralForumThread({
  posts,
  currentUserId,
}: {
  posts: Post[];
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span className="flex items-center gap-1.5">
                  ...
                  {post.author_verified ? <VerifiedBadge size={14} showLabel /> : null}
                </span>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-gray-500">{formatTime(new Date(post.created_at))}</span>
                  {post.author_id === currentUserId && (
                    <GeneralForumPostDeleteButton postId={post.id} imageUrl={post.image_url} />
                  )}
                </div>
              </div>
              <div className="mt-1">
