...
export function GeneralForumTopicList({ topics }: { topics: Topic[] }) {
  if (topics.length === 0) {
    return (
      <div className="rounded-xl border border-gray-700 bg-card p-8 text-center">
        ...
        <p className="mt-1 text-sm text-gray-500">Starte die erste Diskussion mit dem Formular oben.</p>
      </div>
    );
  }
        ...
        <li key={topic.id}>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <h3 className="truncate font-medium text-white">{topic.title}</h3>
