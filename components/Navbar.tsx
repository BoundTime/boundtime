...
export function Navbar({ initialNavData = null }: { initialNavData?: InitialNavData | null }) {
  ...
  return (
    <header className="sticky top-0 z-[60] isolate border-b border-gray-800 bg-background antialiased">
      <nav className="mx-auto flex max-w-6xl items-center justify-between gap-3 py-3 pl-3 pr-3 sm:pl-4 sm:pr-4">
        {user ? (
          <RefreshNavLink
            href="/dashboard"
        ...
        <div className="flex flex-1 items-center justify-end gap-1 sm:gap-2 min-w-0">
          {user ? (
            <>
              ...
              <div className="hidden md:flex md:items-center md:gap-1 md:flex-shrink-0 md:min-w-0">
                <RefreshNavLink
                  href="/dashboard"
                  className={`flex shrink-0 items-center gap-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-background rounded px-1.5 py-0.5 ${nav.isDashboard ? activeLink : inactiveLink}`}
                >
                ...
                <NotificationBell />
                <ChastityNavBadge
                  className="text-sm gap-1.5 rounded px-1.5 py-0.5 text-gray-300 transition-colors hover:text-white"
                  onClick={(e) => {
                    e.preventDefault();
              ...
              <div className="hidden md:flex md:flex-shrink-0 md:items-center md:gap-2 md:border-l md:border-gray-700 md:pl-3">
                ...
                {nick && (
                    <RefreshNavLink
                      href="/dashboard"
                      className={`flex items-center gap-1.5 text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-background rounded ${nav.isDashboard ? activeLink : "text-gray-300 hover:text-white"}`}
                      title="MyBound"
                    >
                    ...
                    <span className="flex items-center gap-1 truncate max-w-[120px]" title={nick ? `Hallo, ${nick}` : undefined}>
                      Hallo, {nick}
                ...
                <button
                  ...
                  className="text-sm font-medium text-accent hover:text-accent-hover transition-colors focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-background rounded whitespace-nowrap"
                >
                  Abmelden
                </button>
              </div>
